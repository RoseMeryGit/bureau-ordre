...
class CourriersSortantsController extends Controller
{
    ...
    public function index(){
        ...
        $sortants = CourriersSortant::all()->where("archiver",false);

        return view('courriers.sortants.index',["sortants"=>$sortants]);
    }
    public function create(){
        ...
        return view('courriers.sortants.create',["user"=>$user,"type_courrier"=>$type_courrier,"service"=>$service,"client"=>$client]); 
    }
    ...
    public function store(Request $req){
        ...
        if($req->file('file')){
            ...
            $file-> move(public_path('Courriers/Sortants'),$filename);
            $data['file']= $filename;
        }
        ...
        return redirect()->route('courriers.sortants.liste')->with('success', 'Le courrier a été enregistré avec succès.');
    ...
    public function consulter($id){
        ...
        return view('courriers.sortants.consulter',["sortants"=>$sortants,"filename_type"=>$filename_type]);
    ...
    public function show_update($id){
        ...
        $courrier_sortant = CourriersSortant::find($id);
        $type_courrier= Type_courrier::all();
        $service= Service_traitant::all();
        $client= Client::all();
        $user= User::all();
        
        $selectedAgents = explode('#', $courrier_sortant->agent);
    
        $selectedActions = explode('/', $courrier_sortant->action);

        return view('courriers.sortants.modifier',["type_courrier"=>$type_courrier,
        "courrier_sortant"=>$courrier_sortant,"user"=>$user,"service"=>$service,
    ...
    public function update(Request $req){
        ...
        $courrier_sortant = CourriersSortant::find($courrier_id);
    
        if ($courrier_sortant) {
            
        $courrier_sortant->reference=$req->reference;
        $courrier_sortant->date_courrier=$req->date_courrier;
        $courrier_sortant->date_envoi=$req->date_envoi;
        $courrier_sortant->client_id=$req->client;
        $courrier_sortant->type_courrier_id=$req->type_courrier;
        $courrier_sortant->service_traitant_id=$req->service_traitant;
        $courrier_sortant->objet=$req->objet;
        
            // Join agents into a single string
            $courrier_sortant->agent = implode('#', $req->agent);
        
            // Join actions into a single string
            $courrier_sortant->action = implode('/', $req->action);
            ...
            if($req->file('file')){
                ...
                $file-> move(public_path('Courriers/Sortants'),$filename);
                $courrier_sortant['file']= $filename;
            }
        ...
        return redirect()->route('courriers.sortants.liste')->with('success', 'Le courrier a été mis à jour avec succès.');
    ...
    public function delete_Courrier_sortant($id)
    {
        ...
        if ($courrier_sortant) {
            ...
            $courrier_sortant->delete();
    
            // Rediriger avec un message de succès
            return redirect()->route('courriers.sortants.liste')->with('success', 'Le courrier a été supprimé avec succès.');
        
        } else {
            // Si le courrier n'existe pas, renvoyer un message d'erreur
            return redirect()->route('courriers.sortants.liste')->with('error', 'Le courrier n\'a pas été trouvé.');
        }
    }
